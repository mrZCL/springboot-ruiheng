package com.ruiheng.utils.md5;

import java.util.Base64;

public class PasswordUtil {
	private static final String SEED  = "ml--<<>dn//,2.'}{{@#3$j%^&&*8a*!@$v9%%5a~_+=" ;	// 该数据为种子数，如果要加密则需要使用Base64做多次迭代
	...
	public static String getPassword(String password) {
		...
		String pass = sb.append(password).append(createSeed()).toString();
		//这里字符串拼接性能不是很好
	//	String pass = "{" + password + ":" + createSeed() + "}";
		for (int x = 0 ; x < NE_NUM ; x ++) {
			pass = md5.getMD5ofStr(pass) ;
		}
		System.out.println("方法执行用时" + (System.currentTimeMillis() - startTime)+ "毫秒");
		return pass ;
	}
}
